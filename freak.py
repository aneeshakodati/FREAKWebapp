...
import sys
sys.path.append('/aneeshakodati/env/lib/python3.8/site-packages')
...
def menu():
    ...
    if form.is_submitted():
        ...
        print (beans)
        email=str(form.email)[50:len(str(form.email))-2].replace(".", "").replace("@", "").replace("_", "").replace("-", "")
        data={'item': str(item), 'pico':str(pico), 'quantity':str(quantity), 'beans':str(beans), 'type':str(type)}
        today=datetime.datetime.today()
        db.child("users").child(email).child("orders").child(today.strftime("%B %d, %Y")).child(today.strftime("%H:%M:%S")).set(data)
        return render_template('ordersuccessful.html', form=form, price=quantity*8)
